...
- `storeFile(path, content)` - Add file to peer's tree (content: string or Uint8Array), returns CID of stored file node
- `createDirectory(path)` - Add directory to peer's tree, returns CID of stored directory node
